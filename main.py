...
def make_time_chart(dataframe: pd.DataFrame, value_col: str, series_col: str | None, title: str, y_axis_title: str):
    ...
    plot_df = plot_df.sort_values(["period_order", series_col] if series_col else ["period_order"], kind="stable")
...
def main():
    ...
    st.title(APP_TITLE)
    st.caption("Carga entre 2 y 12 archivos Excel del mismo formato para analizar precios, nutrientes e ingredientes por periodo.")
    ...
    with tabs[0]:
        ...
        metric_cols[2].metric("Ingredientes detectados", int(ingredients["ingredient_key"].nunique()) if not ingredients.empty else 0)
        metric_cols[3].metric("Nutrientes detectados", int(nutrients["nutrient_name"].nunique()) if not nutrients.empty else 0)
        ...
        if len(file_summary) < MIN_FILES:
            st.warning(f"Para comparar tendencias deben cargarse al menos {MIN_FILES} archivos. Ahora mismo hay {len(file_summary)} válido(s).")

        product_keys, product_labels = get_sorted_unique_options(products, "product_key", "product_display")
        ingredient_keys, ingredient_labels = get_sorted_unique_options(ingredients, "ingredient_key", "ingredient_display") if not ingredients.empty else ([], {})
        ...
        nutrient_scope = nutrients[nutrients["product_key"] == selected_product] if selected_product else nutrients
        nutrient_options = sorted(nutrient_scope["nutrient_name"].dropna().unique().tolist()) if not nutrient_scope.empty else []
        selected_nutrients = selector_cols[1].multiselect("Nutrientes", options=nutrient_options)

        ingredient_scope = ingredients[ingredients["product_key"] == selected_product] if selected_product else ingredients
        ingredient_options = ingredient_scope[["ingredient_key", "ingredient_display"]].drop_duplicates().sort_values("ingredient_display") if not ingredient_scope.empty else pd.DataFrame(columns=["ingredient_key", "ingredient_display"])
        ingredient_option_keys = ingredient_options["ingredient_key"].tolist()
        ingredient_option_labels = dict(zip(ingredient_options["ingredient_key"], ingredient_options["ingredient_display"]))
        ...
        if selected_product:
            ...
            product_series = products[products["product_key"] == selected_product].copy().sort_values("period_order")
            make_time_chart(
                product_series,
            ...
            if selected_nutrients:
                ...
                nutrient_series.sort_values(["period_order", "nutrient_name"], inplace=True)
                make_time_chart(
                    nutrient_series,
                    value_col="level",
                ...
                st.dataframe(
                    nutrient_series[["period_display", "nutrient_name", "level", "source_file"]]
                    .rename(
                        columns={
                            "period_display": "Periodo",
                            "nutrient_name": "Nutriente",
                            "level": "Nivel",
                            "source_file": "Archivo",
                        }
                    ),
                ...
                export_sheets["Nutrientes_seleccionados"] = nutrient_series

            if selected_ingredients:
                st.subheader("Ingredientes seleccionados")
                ingredient_series = ingredients[
                    (ingredients["product_key"] == selected_product) & (ingredients["ingredient_key"].isin(selected_ingredients))
                ].copy()
